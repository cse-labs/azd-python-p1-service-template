...
def test_get_container_service_client(azure_client):
    ...
    azure_client.get_container_service_client = MagicMock(return_value=container_service_client_mock)

    # Test that the method returns the mocked object
    assert azure_client.get_container_service_client("subscription_id") == container_service_client_mock
...
def test_get_gitops_repo_pat_token(azure_client):
    ...
    client_secret = azure_client.get_gitops_repo_pat_token(azd_env_name, subscription_id, resource_group_name)
